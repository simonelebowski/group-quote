...
export default function Summary({
  ...
  return (
    <Card className="md:col-span-2">
      ...
      <div className="space-y-2 text-sm">
        ...
        <Row
          ...
          value={
            editMode ? (
              <input
                type="number"
                className="h-8 w-28 rounded-xl border border-neutral-200 bg-white px-2 text-right text-xs focus:outline-none focus:ring-2 focus:ring-neutral-900/10"
                value={
                  quoteOverrides.basePerStudent ?? pricing.meta.basePerStudent
                }
                onChange={(e) =>
                  setQuoteOverrides((prev) => ({
                    ...
                    basePerStudent:
                      e.target.value === ""
                        ? undefined // go back to default
                        : parseFloat(e.target.value || "0"),
                  }))
                }
              />
            ) : (
              fmt(pricing.meta.basePerStudent, pricing.currency)
            )
          }
        />

        <Row
          label="Base per paying leader"
          value={fmt(pricing.corePayingLeadersTotal, pricing.currency)}
        />
        ...
        {pricing.meta.lessonDelta !== 0 && (
          <Row
            label={`Lesson adjustment per head (${
              pricing.meta.lessonDelta > 0 ? "+" : "-"
            }${Math.abs(pricing.meta.lessonDelta)} lesson${
              Math.abs(pricing.meta.lessonDelta) !== 1 ? "s" : ""
            })`}
            value={fmt(pricing.meta.lessonAdjPerStudent, pricing.currency)}
          />
        )}
        ...
        <Row
          ...
          value={`${students} × ${fmt(
            ...
            pricing.currency
          )} = ${fmt(pricing.coreStudentsTotal, pricing.currency)}`}
        />

        <Row
          label="Core (paying leaders)"
          value={`${pricing.payingLeaders} × ${fmt(
            pricing.perLeaderCore,
            pricing.currency
          )} = ${fmt(pricing.corePayingLeadersTotal, pricing.currency)}`}
        />
        ...
        {arrivalOpt && (
          <Row
            ...
            value={fmt(pricing.meta.arrivalTransferTotal, pricing.currency)}
          />
        )}
        ...
        {departureOpt && (
          <Row
            ...
            value={fmt(pricing.meta.departureTransferTotal, pricing.currency)}
          />
        )}
    ...
    /> */}
        ...
        {/* Custom items */}
        {pricing.customBreakdown && pricing.customBreakdown.length > 0 && (
          <div className="mt-2">
